...
//ARUCO LIBS. THIS ISN'T JOSH GRAFF
//ARUCO MADE THIS FILE AND THE CPP
...
class Aruco_Camera {
  public:
     ...
     */
    Aruco_Camera(cv::Mat cameraMatrix, cv::Mat distorsionCoeff, cv::Size size) throw(cv::Exception);

    void add_points(cv::Point3f point3, cv::Point2f point2);
    bool calibrate();
    cv::Point2d transformto_image(cv::Point3d real_point);
    cv::Point3d transformto_real(cv::Point2d image_point);
    ...
    /**Copy constructor
     */
    Aruco_Camera(const Aruco_Camera &CI);
    ...
    /**Assign operator
    */
    Aruco_Camera &operator=(const Aruco_Camera &CI);
